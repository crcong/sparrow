...
const positionClass = computed(() => {
  switch (props.direction) {
    case 'bottom':
      return `bottom-0 left-0 right-0 ${props.border ? 'border-t' : ''}`
    case 'top':
      return `top-0 left-0 right-0 ${props.border ? 'border-b' : ''}`
    case 'left':
      return `bottom-0 left-0 top-0 ${props.border ? 'border-r' : ''} w-max`
    case 'right':
      return `bottom-0 top-0 right-0 ${props.border ? 'border-l' : ''} w-max`
    default:
      return ''
  }
})
